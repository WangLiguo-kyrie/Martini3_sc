...
intervals=[12000,15000,18000,21000,24000,27000,30000]
...
with open('phase.txt','w') as g:
    ...
    for i in range(6):
        ...
        #Dilute phase
        dilute1=data_interval[(data_interval[:,0]>15)]
        dilute2=data_interval[(data_interval[:,0]<-15)]
        dilute_conc=(np.mean(dilute1[:,1])+np.mean(dilute2[:,1]))*factor/2
        #Dense phase
        dense=data_interval[((data_interval[:,0]>-2)&(data_interval[:,0]<2))]*factor
        print(dense)
